...
2. **Validate** — Run validators against each row (mandatory fields, date/email/phone/postcode formats, valid Australian states and genders, foreign-key integrity, duplicate detection). Errors and warnings are stored per row and displayed in the UI grid with colour coding.
...
- **Two-tier validation severity.** Each `ValidationError` carries either `Error` (blocks ingestion) or `Warning` (informational — row is still ingested) severity. Rows with only warnings are colour-coded yellow in the grid.
...
- No external database required — the app creates a local SQLite file (`CorePractice.db`) next to the executable on first run

### Running the Application

1. Open `CP.Migrator.sln` in Visual Studio.
...
6. Click **Validate** to check all rows. Rows are colour-coded:
